...
今回は SSG フレームワークを [Astro](https://astro.build/)、HeadlessCMS を [Newt](https://www.newt.so/) で実装してみた経験を記事にしてみることにしました。  
...
今回は例としてブログの記事ページみたいなイメージで pages ディレクトリ直下に用意した記事ページで動的ルーティングを行う場合で解説していきます。
...
では、以下でそれぞれを具体的にみていきましょう！

### 1.動的にパラメーターを指定し…

これは特に説明する必要もなく、そのまま`/pages/[slug].astro`と言う感じで pages ディレクトリ直下に動的ルーティングさせたいページをブラケット記法で用意しておくだけです。  
もちろん、ファイル名は必ず`[slug].astro`にする必要はなくて自由にしていただいて問題ないです。  
ただ、今回はわかりやすくするためにこのようにしています。

### 2.getStaticPaths 内で動的ルーティングの処理を行う
...
それで、postData のままではこんな感じのデータ構造になっているので
...
実際にコンテンツの表示に関わる内容である items の内容が使いやすいように `postsContents` を用意しておきます。
...
上記のコードを見ると分かると思いますが、page の中で先ほど取得してきた`postsContents`を展開していって、ブログとして投稿した内容から`[slug]`に入る値が欲しいので、`postsContent.slug`の値を返します。

今 map はで処理されていますが、中身をみてみるとこんな感じになっています。
...
Astro はコンテンツのコンテンツの豊富な Web サイトに特化しているということもあり、ページネーションもデフォルト用意されています。(さすが！)

しかし、ドキュメントを見る限りでは「前のページ」「次のページ」「総ページ数」程度しか得られないため、仕様によっては自分でこの辺りを調整して実装する必要がありそうです。  
この辺りは後ほど解説していこうと思います。

まず、こちらも実装の流れから見てみましょう。

- 1.ページ番号を振りたいファイルを`[...page].astro`みたいな感じでブラケット記法を用いて用意しておく
- 2.getStaticPaths 関数内で paginate()を使ってページを生成
- 2-1.Newt から投稿データを取得してくる
- 2-2.paginate()関数でページネーションの設定をする
- 2-3.前後送り以外のページネーションも

では、以下でそれぞれ具体的にみていきましょう！
...
具体的にどういうことかというと、例えばページネーションを作って 1 ページ目 2 ページ目 3 ページ目の表示を作ることができるようになって、`[page].astro`の場合は`1`,`/2`,`/3`と問題なく作っていくことができますが、トップページは作成されません。  
...
中身を見てみると、表示したいコンテンツを第 1 引数に、1 ページの中で表示したいコンテンツの数を第 2 引数に設定しておきます。  
簡単ですね！
...
最後におまけで公式で用意されている前後送り以外のページネーションじゃ、実務で使えないだろ！という方もいると思うので、[個人的に作ったページネーションのロジック](https://github.com/komosyu4649/frocari/blob/main/src/components/Pagination.astro)があるので、よかったら使ってみてください。

ここでは Astro と Newt の話なので、細かくページネーションのロジックについては触れませんが、使い方としては`Pagination`コンポーネントに先ほど作成した`page`を渡してあげると、すべての数字が展開されて邪魔にならないように...が出て省略されるようにしております。
